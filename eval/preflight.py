...
def check_backend(base_url: str) -> bool:
    """检查后端是否健康。"""
    print("[1/6] 后端健康检查...", end=" ", flush=True)
    try:
...
def check_rag(rag_url: str) -> bool:
    """检查 RAG 服务是否健康 + 内存。"""
    print("[2/6] RAG 服务健康检查...", end=" ", flush=True)
    try:
...
def check_single_case(client, pair: str, fast: bool) -> bool:
    """单 case 端到端测试。"""
    print("[3/6] 单 case 端到端测试...", end=" ", flush=True)
    try:
        # 加载一个 case
...
def check_checkpoint() -> bool:
    """验证 checkpoint 写入机制。"""
    print("[4/6] Checkpoint 写入测试...", end=" ", flush=True)
    try:
...
def check_judge(judge_url: str) -> bool:
    """检查 Judge 服务。"""
    print("[5/6] Judge 服务测试...", end=" ", flush=True)
    try:
