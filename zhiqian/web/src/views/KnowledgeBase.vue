  ...
  <div class="page">
    <el-card shadow="never">
      ...
      <el-form inline @submit.prevent="doSearch">
        ...
        <el-form-item>
          <el-button type="primary" :icon="Search" @click="doSearch" :loading="loading">检索</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-row :gutter="16" style="margin-top:16px">
      ...
      <el-col :span="8">
        <el-card shadow="never" v-if="result?.critique">
          <template #header>Self-RAG 评估</template>
          <el-descriptions :column="1" border>
            <el-descriptions-item label="判定">
              <el-tag :type="verdictType(result.critique.verdict)" v-text="result.critique.verdict" />
            </el-descriptions-item>
            <el-descriptions-item label="综合分"><span v-text="result.critique.score" /></el-descriptions-item>
            <el-descriptions-item label="原因">
              <ul style="padding-left:18px;margin:0">
                <li v-for="(r, i) in result.critique.reasons" :key="i" v-text="r" />
              </ul>
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
...
function verdictType(v: string) {
  return v === 'SUPPORTED' ? 'success' : v === 'PARTIAL' ? 'warning' : 'danger'
}
...
async function doSearch() {
  ...
  try {
    ...
    const resp = await axios.post(`${base}/query`, {
