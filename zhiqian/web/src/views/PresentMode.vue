<!-- v2-step-26 + polish-9: 答辩演示模式。16 张片, 左右键翻页, F 全屏, T 语音 -->
<!-- 模板插值统一用 v-text 指令, 避免 Vue   与平台 URL 压缩冲突 -->
...
const slides = ref<Slide[]>([
  ...
  { title: '选题背景', bullets: ['信创 + 数据库国产化 + AI 代码改造', 'openGauss 迁移是央企刚需'],
    speech: '选题看三个产业驱动: 信创国产化、AI 原生软件、深水难点。' },
  { title: '技术架构', bullets: ['Spring Boot 主服务 + Python RAG + Vue 控制台', 'DeepSeek + BGE-M3 + Qdrant + Langfuse'],
    speech: '整体架构三层, 以反应堆思路安排。' },
  ...
  { title: 'Late Chunking', bullets: ['全文 token embed 之后分块', '上下文保留 → recall +18%'],
    speech: 'Late Chunking 是 Jina 推的分块创新, 国内少有实现。' },
  { title: 'CRAG 补救检索', bullets: ['Self-RAG 评估 → 补救 → 生成', '未知点路由到 web search'],
  ...
  { title: 'GraphRAG 代码知识图', bullets: ['Microsoft GraphRAG 启发', 'Louvain-Lite 社区发现 + local/global 问答'],
    speech: 'GraphRAG 部分, 我们在不依赖 graphrag-toolkit 下自实现。' },
  ...
  { title: 'Temporal Durable Workflow', bullets: ['重试/超时/补偿 原生', '默认 opt-in, ObjectProvider 降级'],
    speech: 'Temporal 是 Uber 开源的持久化工作流, 默认不启以免增加部署负担。' },
  ...
  { title: 'MCP + A2A 协议', bullets: ['MCP: ZhiQian 走出去被调', 'A2A: AgentCard + tasks/sendSubscribe'],
    speech: 'MCP 让 Claude Desktop 能调 ZhiQian, A2A 让 ZhiQian 与别的 Agent 互联。' },
  ...
  { title: '创新点', bullets: ['CRAG/GraphRAG 自主实现', 'pgloader/MTK 适配层 — 不锁定商业', 'MCP + A2A 双协议'],
    speech: '创新点三个: 技术自研、商业开放、生态互联。' },
  { title: '谢谢聆听', bullets: ['github.com/anothersunset/zhixianyunshu', '欢迎提问'],
    speech: '谢谢各位老师, 以上是我们的答辩。' },
])
...
<template>
  <div class="present-root">
    <div class="slide">
      <h1 class="slide-title" v-text="current.title"></h1>
      <ul class="slide-bullets">
        <li v-for="(b, i) in current.bullets" :key="i" v-text="b"></li>
      </ul>
      <div class="speech-note">
        <strong>讲词:</strong> <span v-text="current.speech"></span>
      </div>
    </div>
    <div class="toolbar">
      <el-button size="small" @click="prev" :disabled="idx === 0">« 上页</el-button>
      <span class="progress" v-text="progress"></span>
